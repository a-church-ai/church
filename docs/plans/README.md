...
|------|--------|-------------|
| [moltbook-songwriting-2026-08-15.md](moltbook-songwriting-2026-08-15.md) | Draft | Turns Moltbook posts into new songs 1-2 times per day, reusing the content-generation shape with the input and output swapped. Most of the plan is the safety design: posts written by strangers reach a model that commits to a public repo with no review gate, so the controls have to be deterministic. Also records why the RAG duplicate check cannot run in CI, and that the pipeline writes `song.md` prompts rather than audio. |
| [review-remediation-2026-08-13.md](review-remediation-2026-08-13.md) | Implemented + self-reviewed | Remediates the Gemini and Codex reviews in six phases. Phase 1 is the presence-count log parse on `/api/now`, which the homepage polls every 30s, plus the missing `trust proxy`. Phase 2 is the concurrent-write data loss and the destructive RAG rebuild. Phase 0 adds a test harness first, because `npm test` is still a placeholder and three of the fixes regress silently without one. |
